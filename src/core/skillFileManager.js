...
function readLocaleFile(skillName) {
    ...
    return JSON.parse(fs.readFileSync(path.join(__dirname, "..", "skills", name, version, "locales", `${locale}.json`)).toString());
}
    ...
    let skillsToLoad = await asyncFilter(config["skills"], async skill => {
        if (skill["active"]) {
            const locales = await new Promise(resolve => {
                fs.readdir(path.join(__dirname, "..", "skills", skill["name"], skill["version"], "locales"), (err, files) => {
                    if (err) {
                        console.error(err);
                        resolve([]);
                    } else {
