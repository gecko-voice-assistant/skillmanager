...
const rhasspyPath =
    readFromConfigFile('main')['rhasspy'] || 'http://127.0.0.1:12101';
...
async function postSlots(slotName, alternatives, overwrite = false) {
    ...
    return await postToRhasspyAPI(
        `/api/slots?overwrite_all=${overwrite}`,
        data
    );
}
...
async function registerSkill(skillName) {
    ...
    for (let i in skill['intents']) {
        ...
        sentences.forEach((sentence) => {
            intentStringLines.push(
                `($slots/launch){launch} ${skill['invocation']} ${sentence}`
            );
        });
    }
    ...
    slotsToReplace = [...new Set(slotsToReplace)].filter(
        (slotName) => !!slotName && !defaultSlots.includes(slotName)
    );
